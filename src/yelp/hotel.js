...
exports.updateHotels = async function (hcmsToken, yelpToken) {
  const res = await axios.get("https://api.yelp.com/v3/businesses/search?term=hotels&location=sandy city utah&limit=50", {
    headers: {
  ...
  await updateHCMS(hcmsToken);
};

async function updateHCMS(hcmsToken) {
  console.log("****************UPDATING HCMS CALLED!!! ");
  const res = await axios.put("https://content.civicplus.com/api/content/ut-sandycity/html/f03cd4b7-f35a-43c5-89e6-c4313c30c98a", {
    data: {
      "html-title": {
